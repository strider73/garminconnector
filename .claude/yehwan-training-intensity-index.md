# Yehwan's Training Intensity Index

**Analysis Period:** March 6, 2025 - February 16, 2026 (340 days)
**Data Source:** Active calories from garmin_daily_metrics (reliable indicator of training load)

## Training Intensity Distribution

Based on 242 days with valid active calorie data:

| Intensity Level | Active Calories | Frequency | % of Time | Typical Acute Load |
|-----------------|-----------------|-----------|-----------|-------------------|
| 🛌 **Rest/Recovery** | <300 | 41 days | 16.9% | ~90 (range: 6-986) |
| 🚶 **Light Training** | 300-600 | 53 days | 21.9% | ~71 (range: 10-1541) |
| 🏃 **Moderate Training** | 600-1000 | 57 days | 23.6% | ~315 (range: 4-1508) |
| 💪 **Hard Training** | 1000-1500 | 56 days | 23.1% | ~501 (range: 17-1611) |
| 🔥 **Very Hard Training** | >1500 | 35 days | 14.5% | ~1084 (range: 6-1719) |

## Estimated Court Time by Intensity Level

### 🛌 Rest/Recovery (<300 active calories)
**Court Time:** 0 hours
**Training Split:**
- Morning: Off
- Afternoon: Off

**Typical Activities:**
- Complete rest day
- Light stretching (15-30 mins)
- Recovery walk
- Foam rolling
- No court work

**Frequency:** 1 day per week (17%)

---

### 🚶 Light Training (300-600 active calories)
**Court Time:** 0.5-1.5 hours
**Training Split:**
- Morning: 0-1h (optional)
- Afternoon: 0.5-1.5h

**Typical Activities:**
- Easy groundstroke drills (~400 cal/hr)
- Serve practice (low intensity)
- Movement drills (footwork)
- Light hitting with coach
- Single session day

**Frequency:** 1-2 days per week (22%)

---

### 🏃 Moderate Training (600-1000 active calories)
**Court Time:** 1.5-2.5 hours
**Training Split:**
- Morning: 1-1.5h
- Afternoon: 1-1.5h

**Typical Activities:**
- Morning: Technical drills + serve practice (~500 cal/hr)
- Afternoon: Practice sets or match play (~600 cal/hr)
- Two sessions: 1-1.5h each
- Standard training day

**Frequency:** 2 days per week (24%)

---

### 💪 Hard Training (1000-1500 active calories)
**Court Time:** 2.5-3.5 hours
**Training Split:**
- Morning: 1.5-2h
- Afternoon: 1.5-2h

**Typical Activities:**
- Morning: Hard hitting session + match play (~650 cal/hr)
- Afternoon: Tournament simulation or intense drills
- Two sessions: 1.5-2h each
- May include additional fitness training
- High-intensity training day

**Frequency:** 1-2 days per week (23%)

---

### 🔥 Very Hard Training (>1500 active calories)
**Court Time:** 3-5 hours
**Training Split:**
- Morning: 2-2.5h
- Afternoon: 1.5-2.5h

**Typical Activities:**
- Morning: Full match (~700 cal/hr) or intense training block
- Afternoon: Second match OR hard drilling session
- Tournament days: Multiple matches
- 2-3 sessions per day
- Peak training/competition day

**Frequency:** 1 day per week (15%)

---

## Weekly Training Pattern

**Typical week breakdown (based on actual data):**

| Day Type | Frequency | Court Time | Weekly Total |
|----------|-----------|------------|--------------|
| Rest | 1 day | 0h | 0h |
| Light | 1-2 days | 0.5-1.5h | 1-2h |
| Moderate | 2 days | 1.5-2.5h | 3-5h |
| Hard | 1-2 days | 2.5-3.5h | 3-5h |
| Very Hard | 1 day | 3-5h | 3-5h |

**Weekly Average:** 12-15 hours of court time

This volume matches typical UTR 8 (advanced tournament level) training requirements.

## Training Philosophy

Yehwan's data reveals a **periodized, balanced approach:**

1. **Recovery Built In:** 39% of days are rest/light (<600 cal)
2. **High Intensity When Needed:** 37% of days are hard/very hard (>1000 cal)
3. **Not Constant Grinding:** Varies intensity throughout the week
4. **Tournament Peaks:** Very hard days (>1500 cal) typically tournament/competition days

This approach prevents burnout and allows for proper recovery while maintaining high performance.

## Calorie Burn Reference

For Yehwan (75kg, 20 years old, competitive level):

- **Light hitting/drills:** ~400 cal/hr
- **Moderate intensity practice:** ~500 cal/hr
- **Match play (competitive):** ~650 cal/hr
- **Tournament/intense matches:** ~750 cal/hr

**Note:** Actual calorie burn varies based on:
- Intensity of play (rally length, court coverage)
- Rest intervals between drills/points
- Additional fitness training
- Environmental conditions (heat/humidity increases burn rate)

## Using This Data

**For AI Coaching:**
- Use active calories to assess daily training load
- Compare today's calories to typical ranges for intensity classification
- Multiple consecutive hard days (>1000 cal) = overreaching risk
- 3+ consecutive days >1500 cal = high injury risk

**For Training Planning:**
- Target weekly distribution matches historical pattern
- Don't exceed 2 consecutive very hard days
- Plan rest/light days after hard training blocks
- Monitor acute load trends over 7-day periods

**Red Flags:**
- >1500 cal for 3+ consecutive days
- Acute load >1300 (90th percentile - excessive)
- <300 cal for 7+ consecutive days (detraining risk)
